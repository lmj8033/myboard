...
//   db.collection('post').updateOne({ _id: parseInt(req.body.id) }, {$set : { 할일: req.body.title, 날짜: req.body.date }}, function(에러, 결과){
//     console.log(결과);
//     res.redirect('/list');
//   });
// });
...
// // MY PAGE로 넘어가려면 로그인 여부 확인 (하지 않았다면 로그인 창으로 이동)
...
// // 글 삭제 (작성자가 같을 때만 데이터 모두 삭제되고 작성자가 다르면 새로고침 했을 때 다시 나타남)
// // ( req.body._id 와 req.user._id 를 비교하여 값이 다를 때 '작성자가 다릅니다' 창을 띄우고 싶은데 어떻게 할까용)
// app.delete('/delete', function(req, res) {
//   console.log('삭제요청들어옴')
//   console.log(req.body);
//   req.body._id = parseInt(req.body._id);
